...
async function analyzeGitHubRepository(repoUrl) {
    try {
        ...
        for (const contributor of contributorsToProcess) {
            try {
                ...
                if (userResponse.ok) {
                    ...
                    const userAgeInDays = Math.floor((Date.now() - new Date(userCreatedAt).getTime()) / (1000 * 60 * 60 * 24));
                    totalContributorAge += userAgeInDays;
                    processedContributors++;
                }
            }
            catch (error) {
                console.warn(`Could not fetch details for contributor: ${contributor.login}`, error.message);
            }
        }
        const averageContributorAgeInDays = processedContributors > 0 ? Math.floor(totalContributorAge / processedContributors) : null;
        ...
        if (dependencyAnalysis.parsedDependencies.length > 0) {
            try {
                ...
                if (ecosystem) {
                    ...
                    for (const batch of batches) {
                        const vulnerabilityPromises = batch.map(async (dep) => {
                            try {
                                ...
                                console.warn(`Error checking vulnerability for ${dep.name}@${dep.version}:`, err.message);
                                return [];
                            }
                        });
                    ...
                    const mediumSeverityVulns = osvVulnerabilities.filter(v => v.severity === 'MEDIUM').length;
        ...
        if (dependencyAnalysis.parsedDependencies.length > 0) {
            try {
                ...
                if (ecosystem) {
                    ...
                    for (const batch of batches) {
                        const versionPromises = batch.map(async (dep) => {
                            try {
                                ...
                                if (ecosystem === 'npm') {
                                    const npmResponse = await fetch(`https://registry.npmjs.org/${encodeURIComponent(dep.name)}/latest`);
                                    if (npmResponse.ok) {
                                ...
                                // Check PyPI for Python packages
                                    ...
                                    const pypiResponse = await fetch(`https://pypi.org/pypi/${encodeURIComponent(dep.name)}/json`);
                                    if (pypiResponse.ok) {
                                ...
                                if (latestVersion && dep.version) {
                                    ...
                                    if (semver_1.default.valid(semver_1.default.coerce(cleanCurrentVersion)) &&
                                        semver_1.default.valid(semver_1.default.coerce(cleanLatestVersion))) {
                                        // Calculate version difference
                                        const currentSemver = semver_1.default.coerce(cleanCurrentVersion);
                                        const latestSemver = semver_1.default.coerce(cleanLatestVersion);
                                        if (currentSemver && latestSemver) {
                                            const isMajorBehind = semver_1.default.major(latestSemver) > semver_1.default.major(currentSemver);
                                            const isMinorBehind = semver_1.default.major(latestSemver) === semver_1.default.major(currentSemver) &&
                                                semver_1.default.minor(latestSemver) > semver_1.default.minor(currentSemver);
                                            const isPatchBehind = semver_1.default.major(latestSemver) === semver_1.default.major(currentSemver) &&
                                                semver_1.default.minor(latestSemver) === semver_1.default.minor(currentSemver) &&
                                                semver_1.default.patch(latestSemver) > semver_1.default.patch(currentSemver);
                                            const versionsBehind = {
                                                major: semver_1.default.major(latestSemver) - semver_1.default.major(currentSemver),
                                                minor: isMinorBehind
                                                    ? semver_1.default.minor(latestSemver) - semver_1.default.minor(currentSemver)
                                                    : 0,
                                                patch: isPatchBehind
                        ...
                        const outdatedInBatch = batchResults.filter((item) => item !== null);
                    ...
                    dependencyAnalysis.majorOutdatedCount = dependencyAnalysis.outdatedDependencies.filter(dep => dep.isMajorBehind).length;
                    dependencyAnalysis.minorOutdatedCount = dependencyAnalysis.outdatedDependencies.filter(dep => !dep.isMajorBehind && dep.isMinorBehind).length;
                }
            }
            catch (error) {
        ...
        const contributingGuidelinesResponse = await fetchWithAuth(`${apiUrl}/contents/CONTRIBUTING.md`);
            ...
            try {
                ...
                if (readmeData.content && readmeData.size && readmeData.size < 100000) {
                    ...
                    for (const paragraph of paragraphs) {
                        ...
                        if (trimmed.startsWith('#') || // Skip headings
        ...
        if (releasesResponse.ok) {
            ...
            if (releases.length > 0) {
                ...
                releaseInfo.daysSinceLastRelease = Math.floor((Date.now() - releaseInfo.latestReleaseDate.getTime()) / (1000 * 60 * 60 * 24));
        ...
        // --- Fetch Open Issues ---
        const issuesResponse = await fetchWithAuth(`${apiUrl}/issues?state=open&sort=created&direction=asc`);
        ...
        if (actualOpenIssues && actualOpenIssues.length > 0) {
            ...
            const closedIssuesResponse = await fetchWithAuth(`${apiUrl}/issues?state=closed&sort=updated&direction=desc&per_page=30`);
            if (closedIssuesResponse.ok) {
                    ...
                    if (!issue.pull_request) {
                        ...
                        if (commentsResponse.ok) {
                            ...
                            if (comments.length > 0) {
                                ...
                                const responseTimeHours = Math.floor((firstResponseDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60));
                ...
                if (issuesWithComments.length > 0) {
                    const totalResponseTime = issuesWithComments.reduce((sum, issue) => sum + issue.responseTimeHours, 0);
                    responseTimeMetrics = {
        ...
        else if (releaseInfo.daysSinceLastRelease !== null &&
        ...
        if (dependencyAnalysis.highSeverityCount !== null && dependencyAnalysis.highSeverityCount > 0) {
            riskScore += 3;
            riskFactors.push(`${dependencyAnalysis.highSeverityCount} high severity vulnerabilities found.`);
        }
        if (dependencyAnalysis.mediumSeverityCount !== null &&
            dependencyAnalysis.mediumSeverityCount > 0) {
            riskScore += 2;
            riskFactors.push(`${dependencyAnalysis.mediumSeverityCount} medium severity vulnerabilities found.`);
        }
        if (dependencyAnalysis.lowSeverityCount !== null && dependencyAnalysis.lowSeverityCount > 0) {
            riskScore += 1;
            riskFactors.push(`${dependencyAnalysis.lowSeverityCount} low severity vulnerabilities found.`);
        }
        ...
        if (dependencyAnalysis.majorOutdatedCount > 5) {
            riskScore += 3;
            riskFactors.push(`Many dependencies are severely outdated (${dependencyAnalysis.majorOutdatedCount} major versions behind).`);
        }
        else if (dependencyAnalysis.majorOutdatedCount > 0) {
            riskScore += 2;
            riskFactors.push(`Some dependencies are severely outdated (${dependencyAnalysis.majorOutdatedCount} major versions behind).`);
        }
        if (dependencyAnalysis.minorOutdatedCount > 10) {
            riskScore += 1;
            riskFactors.push(`Many dependencies need minor version updates (${dependencyAnalysis.minorOutdatedCount} minor versions behind).`);
        }
...
| Avg. Contributor Account Age | ${averageContributorAgeInYears ? `${averageContributorAgeInYears} years` : 'N/A'} |
...
| Last Activity | ${daysSinceLastCommit !== null
...
| Total Dependencies | ${dependencyAnalysis.dependenciesCount > 0 ? dependencyAnalysis.dependenciesCount : 'N/A'} |
| Major Version Outdated | ${dependencyAnalysis.majorOutdatedCount > 0
            ? `⚠️ ${dependencyAnalysis.majorOutdatedCount}`
            : dependencyAnalysis.parsedDependencies.length > 0
                ? '✅ 0'
                : 'N/A'} |
| Minor Version Outdated | ${dependencyAnalysis.minorOutdatedCount > 0
            ? `ℹ️ ${dependencyAnalysis.minorOutdatedCount}`
            : dependencyAnalysis.parsedDependencies.length > 0
                ? '✅ 0'
                : 'N/A'} |
| Vulnerability Alerts | ${dependencyAnalysis.alertsEnabled ? '✅ Enabled' : '❌ Disabled/Not Available'} |
| High Severity Vulnerabilities | ${dependencyAnalysis.highSeverityCount !== null ? dependencyAnalysis.highSeverityCount : 'N/A'} |
| Medium Severity Vulnerabilities | ${dependencyAnalysis.mediumSeverityCount !== null
            ? dependencyAnalysis.mediumSeverityCount
            : 'N/A'} |
| Low Severity Vulnerabilities | ${dependencyAnalysis.lowSeverityCount !== null ? dependencyAnalysis.lowSeverityCount : 'N/A'} |
...
${dependencyAnalysis.outdatedDependencies
                .map(dep => `| ${dep.name} | ${dep.currentVersion} | ${dep.latestVersion} | ${dep.updateUrgency === 'high'
                ? '🔴 High'
...
| Average Response Time | ${responseTimeMetrics
...
| Metric | Value |
|--------|-------|
| Formal Releases | ${releaseInfo.hasReleases ? `✅ (${releaseInfo.releaseCount} found)` : '❌ None found'} |
| Latest Release | ${releaseInfo.latestReleaseDate
            ? `${releaseInfo.daysSinceLastRelease} days ago (${releaseInfo.latestReleaseDate.toLocaleDateString()})`
            : 'N/A'} |
...
function extractFixedVersions(vulnerability) {
    if (!vulnerability.affected || !vulnerability.affected.length)
        return 'unknown';
    const fixedVersions = [];
